...
class ANN(nn.Module):
    ...
    def init_indices(self):
        vip = VIP()
        selector = SPA(n_features_to_select=self.shortlist)
        vip.fit(self.dataset_object.get_bs_train_x(), self.dataset_object.get_bs_train_y())
        mask = vip.vips_ > 0.3
        selector.fit(self.dataset_object.get_bs_train_x(), self.dataset_object.get_bs_train_y(), mask=mask)
        support = selector.get_support(indices=True)
        indices = torch.tensor(support)
...
class Algorithm_slspa(Algorithm):
