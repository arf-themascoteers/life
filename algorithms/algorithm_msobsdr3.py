...
class Agent(nn.Module):
    def __init__(self, target_size, class_size, classification, offset, start, r1, r2, last=False):
        super().__init__()
...
class ANN(nn.Module):
    def __init__(self, target_size, class_size, original_size, classification):
        ...
        self.agents = nn.ModuleList(
            [Agent(self.target_size, self.class_size, self.classification, band_unit, i, rs[i], rs[i+1], last=(i==self.num_agents-1)) for i in range(self.num_agents)]
        )
...
class Algorithm_msobsdr3(Algorithm):
